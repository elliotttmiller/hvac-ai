...
**Target Infrastructure:** Ray Serve (Local GPU $\rightarrow$ Cloud Cluster)
...
2.  **Node A (Object Detector):** Detects component geometry (OBB).
3.  **Node B (Geometry Engine):** Maps coordinates to the *original high-res image*, performs perspective transforms to "un-rotate" regions of interest.
4.  **Node C (Text Extractor):** Reads text from the straightened crops.
5.  **Node D (Fusion Layer):** Merges spatial data and text data into a single JSON response.
...
*   **Logic:** Implement the **Perspective Transform Pipeline**:
    1.  Accept OBB parameters (`x, y, w, h, rotation`) + Original Image.
    2.  Calculate the 4 corner points.
    3.  Warp/Rotate the crop to be perfectly horizontal (0 degrees).
    4.  Apply grayscale/thresholding for OCR contrast enhancement.
